...
import { getFumigationTrackingByContainerAndNotice } from "@/lib/fumigation-tracking"
import type { FumigationTracking } from "@/lib/fumigation-tracking"

export function ContainerTracking() {
  const [containerNumber, setContainerNumber] = useState("")
  const [noticeId, setNoticeId] = useState("")
  const [searchResult, setSearchResult] = useState<FumigationTracking | null>(null)
  const [isSearching, setIsSearching] = useState(false)
  const [notFound, setNotFound] = useState(false)

  const handleSearch = async () => {
    if (!containerNumber.trim() || !noticeId.trim()) {
      alert("Mohon masukkan nomor kontainer dan notice ID")
      return
    }
    ...
    try {
      ...
      const result = getFumigationTrackingByContainerAndNotice(
        containerNumber.toUpperCase().trim(),
        noticeId.toUpperCase().trim(),
      )
      ...
      alert("Terjadi kesalahan saat mencari data kontainer")
    } finally {
  ...
  return (
    <Card className="bg-white border-0 shadow-lg">
      <CardHeader>
        ...
        <CardDescription>Masukkan nomor kontainer dan notice ID untuk melacak progress fumigasi</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                onClick={handleReset}
                disabled={isSearching}
                className="text-prana-gray border-prana-gray hover:bg-prana-gray hover:text-white bg-transparent"
              >
                Reset
              </Button>
            </div>
        ...
        {notFound && (
          <div className="text-center py-8">
            ...
            <h3 className="text-lg font-semibold text-prana-navy mb-2">Data Tidak Ditemukan</h3>
            <p className="text-prana-gray mb-4">
              Kontainer dengan nomor <strong>{containerNumber}</strong> dan Notice ID <strong>{noticeId}</strong> tidak
              ditemukan dalam sistem.
            </p>
            <p className="text-sm text-prana-gray">
              Pastikan nomor kontainer dan notice ID yang Anda masukkan sudah benar, atau hubungi admin untuk informasi
              lebih lanjut.
            </p>
          </div>
        )}
        ...
        {!searchResult && !notFound && (
          <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
            <h4 className="font-semibold text-prana-navy mb-2">Cara Menggunakan Tracking</h4>
            <ul className="text-sm text-prana-gray space-y-1">
              ...
              <li>• Informasi akan menampilkan status real-time dari proses fumigasi</li>
            </ul>
          </div>
        )}
      </CardContent>
